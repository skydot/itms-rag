...
def get_vehicle_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=vehicle")
        return chunks

    try:
        cursor = conn.cursor()
        module = "vehicle"
        roles = ["principal", "admin", "vehicle_staff"]

        tables_to_check = [
            ("vehicle_registers", "VEHICLE REGISTER RECORD"),
            ("vehicle_masters", "VEHICLE MASTER RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "vehicle_number", "vehicle_type", "driver_name", "capacity", "status", "trip_date", "purpose", "from_place", "to_place", "distance"
                    ]
        ...
        logger.error(f"[Chunker] module=vehicle error: {e}")
    finally:
        conn.close()
