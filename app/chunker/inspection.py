...
def get_inspection_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=inspection")
        return chunks

    try:
        cursor = conn.cursor()
        module = "inspection"
        roles = ["principal", "admin", "inspection_staff"]

        tables_to_check = [
            ("inspection_notes", "INSPECTION NOTE RECORD"),
            ("inspection_description", "INSPECTION ACTION ITEM RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "title", "from_date", "to_date", "short_desc", "created_by", "copy_to", "sub_ref", "insp_id", "description", "faculty_id", "in_type", "i_status", "remarks_log", "remarks", "status"
                    ]
        ...
        logger.error(f"[Chunker] module=inspection error: {e}")
    finally:
        conn.close()
