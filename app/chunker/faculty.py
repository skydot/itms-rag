...
def get_faculty_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=faculty")
        return chunks
    ...
    try:
        ...
        tables_to_check = [
            ("vl_management", "VISITING LECTURER RECORD"),
            ("vl_description", "VL DESCRIPTION RECORD"),
            ("feed_que_vls", "FACULTY FEEDBACK RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "faculty_name", "vl_name", "department_id", "designation_id", "status", "faculty_type", "subject_id", "course_id", "rating"
                    ]
        ...
        logger.error(f"[Chunker] module=faculty error: {e}")
    finally:
        conn.close()
