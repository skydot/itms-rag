...
def get_field_study_tour_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=field_study_tour")
        return chunks

    try:
        cursor = conn.cursor()
        module = "field_study_tour"
        roles = ["principal", "admin", "course_coordinator"]

        tables_to_check = [
            ("study_tour", "STUDY TOUR RECORD"),
            ("field_training", "FIELD TRAINING RECORD"),
            ("filled_training_data", "FIELD TRAINING DATA RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "tour_name", "place", "from_date", "to_date", "course_id", "status", "transport_mode", "zone_id", "division_id", "remark"
                    ]
        ...
        logger.error(f"[Chunker] module=field_study_tour error: {e}")
    finally:
        conn.close()
