...
def get_timetable_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=timetable")
        return chunks

    try:
        cursor = conn.cursor()
        module = "timetable"
        roles = ["principal", "admin", "course_coordinator", "trainer", "training_staff"]

        tables_to_check = [
            ("time_masters", "TIMETABLE MASTER RECORD"),
            ("tt_designs", "TIMETABLE DESIGN RECORD"),
            ("tt_designs_daywise", "TIMETABLE DAYWISE RECORD"),
            ("sessions", "SESSION RECORD"),
            ("class_rooms", "CLASSROOM RECORD"),
            ("topics", "TOPIC RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "course_id", "batch_id", "subject_id", "topic_id", "faculty_id", "session_id", "room_id", "class_room_id", "class_room",
                        "date", "day", "start_time", "end_time", "topic_name", "session_name", "status"
                    ]
        ...
        logger.error(f"[Chunker] module=timetable error: {e}")
    finally:
        conn.close()
