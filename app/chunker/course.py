...
def get_course_chunks(office_id: int | None = None, limit: int | None = None):
    chunks = []
    conn = get_connection()
    if not conn:
        logger.warning("[Chunker] No DB connection for module=course")
        return chunks

    try:
        cursor = conn.cursor()
        module = "course"
        roles = ["principal", "admin", "course_coordinator", "training_staff"]

        tables_to_check = [
            ("courses", "COURSE MASTER RECORD"),
            ("training_calendars", "TRAINING CALENDAR / BATCH RECORD"),
            ("subjects", "COURSE SUBJECT RECORD"),
            ("course_groups", "COURSE GROUP RECORD")
        ]
        ...
        for table_name, record_type in tables_to_check:
            if table_exists(cursor, table_name):
                ...
                for row in rows:
                    ...
                    text = f"{record_type}\n"
                    safe_fields = [
                        "course_name", "batch", "from_date", "to_date", "status", "course_code", "course_type",
                        "subject_name", "subject_code", "group_name", "capacity", "coordinator_id", "ct_id"
                    ]
        ...
        logger.error(f"[Chunker] module=course error: {e}")
    finally:
        conn.close()
