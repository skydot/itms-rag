import os
"""Timetable query executor — runs final parameterized SQL for timetable guided flows.

Uses existing response_mode_service and report_service for output formatting.
"""

from typing import Dict, Optional
from datetime import datetime, timedelta
from app.services.db_service import get_connection
from app.services.response_mode_service import detect_response_mode
from app.services.report_service import generate_report
from app.services.llm_service import format_answer


# Sensitive columns to strip from chat output
_SENSITIVE_COLS = {
    'id', 'user_id', 'course_id', 'office_id', 'password',
    'created_by', 'updated_by', 'created_at', 'updated_at', 'status',
    'aadhar', 'uan', 'pf_no', 'bank_acc', 'ifsc_code', 'android_id',
    'permanent_identity', 'photo', 'signature', 'email', 'office_email',
    'whatsapp_number', 'emergency_numbers', 'office_mobile', 'emg_mobile_no',
    'present_address', 'permanent_address', 'resi_address', 'pass_file',
    'user_log', 'room_log', 'attachment', 'hrms_id',
}


def _clean_val(v):
    from decimal import Decimal
    import datetime
    if isinstance(v, Decimal):
        return float(v)
    if isinstance(v, (datetime.date, datetime.datetime, datetime.timedelta)):
        return str(v)
    return v


def _format_rows_for_chat(rows: list, max_rows: int = 15, total_count: Optional[int] = None) -> str:
    """Convert rows to readable text for LLM formatting."""
    if not rows:
        return "No data found."
    if len(rows) == 1 and len(rows[0]) == 1:
        k = list(rows[0].keys())[0]
        v = _clean_val(rows[0][k])
        return f"{k}: {'N/A' if v is None else str(v)}"

    limited = rows[:max_rows]
    lines = []
    for i, row in enumerate(limited, 1):
        parts = []
        for k, v in row.items():
            if k.lower() not in _SENSITIVE_COLS:
                v = _clean_val(v)
                val = "None / 0" if v is None else str(v)
                label = k.replace('_', ' ').title()
                parts.append(f"{label}: {val}")
        if parts:
            lines.append(f"{i}. " + " | ".join(parts))

    actual_total = total_count if total_count is not None else len(rows)
    summary = f"Total count: {actual_total}"
    if actual_total > max_rows:
        summary += f"\n(showing top {max_rows} to avoid text overload. Use 'show list' for full report)"
    return summary + "\n" + "\n".join(lines)


def execute_timetable_guided_query(
    flow_id: str,
    slots: dict,
    office_id: int,
    role: str,
    session_id: str = None,
    user_question: str = "",
    base_url: str = os.getenv("API_BASE_URL", ""),
) -> dict:
    """Execute the final guided query and return formatted result."""
    try:
        print(f"[Timetable Guided] Executing: {flow_id} with slots: {slots}")
        handler = {
            "today_timetable": _exec_today_timetable,
            "tomorrow_timetable": _exec_tomorrow_timetable,
            "date_wise_timetable": _exec_date_wise_timetable,
            "course_timetable": _exec_course_timetable,
            "faculty_timetable": _exec_faculty_timetable,
            "subject_timetable": _exec_subject_timetable,
            "classroom_timetable": _exec_classroom_timetable,
            "session_timetable": _exec_session_timetable,
            "timetable_summary": _exec_timetable_summary,
            "free_slots": _exec_free_slots,
        }.get(flow_id)
        if not handler:
            return None
        return handler(slots, office_id, user_question, session_id, base_url)
    except Exception as e:
        print(f"[Timetable Guided] Error executing {flow_id}: {e}")
        return {"type": "text", "message": f"Error retrieving timetable data: {str(e)}"}


def _build_response(rows, question, office_id, session_id, base_url,
                     force_report=False, force_chat=False, total_count=None):
    row_count = len(rows)
    actual_count = total_count if total_count is not None else row_count

    if actual_count == 0:
        return {"type": "text", "message": "No matching timetable data found for your request."}

    import re
    if not force_report and re.search(r"how\s+many|total|count", question.lower()):
        force_chat = True

    if force_chat:
        mode = "chat"
    elif force_report:
        mode = "report"
    else:
        mode = detect_response_mode(
            user_question=question,
            result_type="list" if actual_count > 1 else "single",
            row_count=actual_count,
        )

    if mode == "report" and row_count > 1:
        report = generate_report(
            module_name="timetable",
            title="Timetable Report",
            user_question=question,
            rows=rows,
            office_id=office_id,
            session_id=session_id,
        )
        full_url = base_url.rstrip("/") + report["url"]
        ttl = report["ttl_seconds"]
        if ttl < 60:
            exp = f"{ttl} seconds"
        elif ttl < 3600:
            exp = f"{ttl // 60} minute{'s' if ttl // 60 != 1 else ''}"
        else:
            exp = f"{ttl // 3600} hour{'s' if ttl // 3600 != 1 else ''}"

        if total_count and total_count > row_count:
            count_msg = f"Found {total_count} records (showing {report['row_count']} in report)."
        else:
            count_msg = f"Found {report['row_count']} records for your request."

        answer = (
            f"{count_msg}\n\n"
            f"Open full report: {full_url}\n\n"
            f"This report link will expire in {exp}."
        )
        return {
            "type": "text",
            "message": answer,
            "report_url": full_url,
            "row_count": report["row_count"],
            "response_mode": "report",
            "expires_at": report["expires_at"],
        }
    else:
        formatted_text = _format_rows_for_chat(rows, total_count=total_count)
        answer = format_answer(question, formatted_text)
        return {"type": "text", "message": answer}


def _resolve_date_sql(date_slot: str) -> tuple:
    if not date_slot:
        return "", []
    if date_slot == "today":
        return " AND tm.tm_date = CURDATE()", []
    if date_slot == "tomorrow":
        return " AND tm.tm_date = DATE_ADD(CURDATE(), INTERVAL 1 DAY)", []
    if date_slot == "yesterday":
        return " AND tm.tm_date = DATE_SUB(CURDATE(), INTERVAL 1 DAY)", []
    if date_slot == "last_7_days":
        return " AND tm.tm_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND tm.tm_date <= CURDATE()", []
    if date_slot == "ALL":
        return "", []
    
    return " AND tm.tm_date = %s", [date_slot]


def _build_core_query(where_clause: str) -> str:
    return f"""
        SELECT tm.tm_date AS date, tm.start_time, tm.end_time,
               c.course_name, tc.course_batch,
               tm.topic_name, s.subject_name,
               u.name AS faculty_name,
               cr.class_name AS classroom,
               ses.session AS session_name
        FROM time_masters tm
        JOIN training_calendars tc ON tc.id = tm.course_id
        JOIN courses c ON c.id = tc.ct_id
        LEFT JOIN users u ON u.id = tm.desi_user_id
        LEFT JOIN subjects s ON s.id = tm.cs_id
        LEFT JOIN class_rooms cr ON cr.id = tm.class_id
        LEFT JOIN sessions ses ON ses.id = tm.session_id
        WHERE tm.office_id = %s AND tm.status = 1
        {where_clause}
        ORDER BY tm.tm_date, tm.start_time
    """


# ═══════════════════════════════════════════════════════════════════
# Individual executor handlers
# ═══════════════════════════════════════════════════════════════════

def _exec_today_timetable(slots, office_id, question, session_id, base_url):
    c_id = slots.get("course_id")
    c_sql = " AND tm.course_id = %s" if c_id else ""
    params = [office_id, c_id] if c_id else [office_id]
    
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.tm_date = CURDATE()" + c_sql) + " LIMIT 300"
        cur.execute(sql, params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_tomorrow_timetable(slots, office_id, question, session_id, base_url):
    c_id = slots.get("course_id")
    c_sql = " AND tm.course_id = %s" if c_id else ""
    params = [office_id, c_id] if c_id else [office_id]
    
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.tm_date = DATE_ADD(CURDATE(), INTERVAL 1 DAY)" + c_sql) + " LIMIT 300"
        cur.execute(sql, params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_date_wise_timetable(slots, office_id, question, session_id, base_url):
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query(d_sql) + " LIMIT 500"
        cur.execute(sql, [office_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_course_timetable(slots, office_id, question, session_id, base_url):
    c_id = slots.get("course_id")
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    if not c_id:
        return {"type": "text", "message": "Please specify a course."}
        
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.course_id = %s " + d_sql) + " LIMIT 500"
        cur.execute(sql, [office_id, c_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_faculty_timetable(slots, office_id, question, session_id, base_url):
    u_id = slots.get("user_id")
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    if not u_id:
        return {"type": "text", "message": "Please specify a faculty member."}
        
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.desi_user_id = %s " + d_sql) + " LIMIT 300"
        cur.execute(sql, [office_id, u_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_subject_timetable(slots, office_id, question, session_id, base_url):
    s_id = slots.get("subject_id")
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    if not s_id:
        return {"type": "text", "message": "Please specify a subject."}
        
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.cs_id = %s " + d_sql) + " LIMIT 300"
        cur.execute(sql, [office_id, s_id] + d_params)
        rows = cur.fetchall()
        
        force_chat = detect_response_mode(question) == "chat"
        # If user asks "when is next", enforce chat mode and limit 1.
        import re
        if re.search(r"\b(next|upcoming|when)\b", question.lower()) and not rows:
            # Maybe search upcoming
            sql = _build_core_query("AND tm.cs_id = %s AND tm.tm_date >= CURDATE()") + " LIMIT 5"
            cur.execute(sql, [office_id, s_id])
            rows = cur.fetchall()
            force_chat = True
            
        return _build_response(rows, question, office_id, session_id, base_url, force_chat=force_chat)
    finally:
        conn.close()


def _exec_classroom_timetable(slots, office_id, question, session_id, base_url):
    cr_id = slots.get("classroom_id")
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    if not cr_id:
        return {"type": "text", "message": "Please specify a classroom."}
        
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.class_id = %s " + d_sql) + " LIMIT 300"
        cur.execute(sql, [office_id, cr_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_session_timetable(slots, office_id, question, session_id, base_url):
    sess_id = slots.get("session_id")
    d_slot = slots.get("date", "ALL")
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    if not sess_id:
        return {"type": "text", "message": "Please specify a session."}
        
    conn = get_connection()
    try:
        cur = conn.cursor()
        sql = _build_core_query("AND tm.session_id = %s " + d_sql) + " LIMIT 300"
        cur.execute(sql, [office_id, sess_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_timetable_summary(slots, office_id, question, session_id, base_url):
    d_slot = slots.get("date", "today")
    if d_slot == "ALL": d_slot = "today"
    d_sql, d_params = _resolve_date_sql(d_slot)
    
    group_by = slots.get("group_by")
    conn = get_connection()
    try:
        cur = conn.cursor()
        
        if not group_by:
            # Overall count summary
            sql = f"""
                SELECT COUNT(tm.id) AS total_lectures,
                       COUNT(DISTINCT tm.course_id) AS distinct_courses,
                       COUNT(DISTINCT tm.desi_user_id) AS distinct_faculty
                FROM time_masters tm
                WHERE tm.office_id = %s AND tm.status = 1 {d_sql}
            """
            cur.execute(sql, [office_id] + d_params)
            rows = cur.fetchall()
            return _build_response(rows, question, office_id, session_id, base_url, force_chat=True)
            
        group_fields = {
            "course": ("c.course_name, tc.course_batch", "c.course_name, tc.course_batch"),
            "faculty": ("u.name AS faculty_name", "u.name"),
            "subject": ("s.subject_name", "s.subject_name"),
            "classroom": ("cr.class_name", "cr.class_name"),
            "session": ("ses.session AS session_name", "ses.session"),
        }
        select_col, group_col = group_fields.get(group_by, group_fields["course"])
        
        sql = f"""
            SELECT {select_col}, COUNT(tm.id) AS lecture_count
            FROM time_masters tm
            JOIN training_calendars tc ON tc.id = tm.course_id
            JOIN courses c ON c.id = tc.ct_id
            LEFT JOIN users u ON u.id = tm.desi_user_id
            LEFT JOIN subjects s ON s.id = tm.cs_id
            LEFT JOIN class_rooms cr ON cr.id = tm.class_id
            LEFT JOIN sessions ses ON ses.id = tm.session_id
            WHERE tm.office_id = %s AND tm.status = 1 {d_sql}
            GROUP BY {group_col}
            ORDER BY lecture_count DESC
            LIMIT 100
        """
        cur.execute(sql, [office_id] + d_params)
        rows = cur.fetchall()
        return _build_response(rows, question, office_id, session_id, base_url)
    finally:
        conn.close()


def _exec_free_slots(slots, office_id, question, session_id, base_url):
    return {
        "type": "text", 
        "message": "Free slot calculation needs precise session timing and occupied timetable records. I found timetable records but cannot reliably calculate free slots from the current schema data without additional mapping."
    }
